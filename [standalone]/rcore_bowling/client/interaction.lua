Citizen.CreateThread(function()
    AddTextEntry('R_B_REGLANE', string.format(Config.Text.REGISTER_LANE, '', ''))
    AddTextEntry('R_B_JOILANE', string.format(Config.Text.JOIN_LANE, ''))
                    ...
                    if not lane.gameState or lane.gameState == 'registration' then
                        ...
                        if #(pedCoords - lane.InteractionPos) < 1.4 then
                            if Lanes[laneName].state then
                                if LaneJob and LaneJob[Lanes[laneName].Place] then
                                    AddTextEntry('R_B_JOILANE_PAID', string.format(Config.Text.JOIN_LANE, '~g~$' .. LaneJob[Lanes[laneName].Place].price .. '~s~ '))
                                    DisplayHelpTextThisFrame('R_B_JOILANE_PAID', true)
                                else
                                    DisplayHelpTextThisFrame('R_B_JOILANE', true)
                                end
                            else
                                if LaneJob and LaneJob[Lanes[laneName].Place] then
                                    local feePrice = '~g~$' .. LaneJob[Lanes[laneName].Place].price .. '~s~ '
                                    AddTextEntry('R_B_REGLANE_PAID', string.format(Config.Text.REGISTER_LANE, feePrice, feePrice))
                                    DisplayHelpTextThisFrame('R_B_REGLANE_PAID', true)
                                else
