local Poles, Dancers, polelist, dancerslist = {}, {}, {}, {}
...
RegisterNetEvent('jim-vanillaunicorn:Dance', function(data) local Ped = PlayerPedId()
	local canDance = true
	local dancing = false
	...
	while dancing do
		if dancing then
			if IsControlJustReleased(0, 202) then
				if IsEntityPlayingAnim(Ped, "mini@strip_club@pole_dance@pole_dance1", "pd_dance_01", 3) or
				IsEntityPlayingAnim(Ped, "mini@strip_club@pole_dance@pole_dance2", "pd_dance_02", 3) or
				IsEntityPlayingAnim(Ped, "mini@strip_club@pole_dance@pole_dance3", "pd_dance_03", 3) then
					dancing = false
					ClearPedTasks(Ped)
		...
		Wait(5)
		if not IsEntityPlayingAnim(Ped, "mini@strip_club@pole_dance@pole_dance1", "pd_dance_01", 3) and
			not IsEntityPlayingAnim(Ped, "mini@strip_club@pole_dance@pole_dance2", "pd_dance_02", 3) and
			not IsEntityPlayingAnim(Ped, "mini@strip_club@pole_dance@pole_dance3", "pd_dance_03", 3) then
				dancing = false
				for i = 1, 3 do unloadAnimDict("mini@strip_club@pole_dance@pole_dance"..i) end
			end
		end
