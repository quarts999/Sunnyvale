local vseat, Chairs, chairlist = 0, {}, {}
...
RegisterNetEvent('jim-vanillaunicorn:Chair', function(data)
	...
	--local cam = createTempCam(data.entity, PlayerPedId())
	...
	if canSit then
		...
		TaskStartScenarioAtPosition(PlayerPedId(), "PROP_HUMAN_SEAT_CHAIR_MP_PLAYER", data.loc.x, data.loc.y, data.loc.z-0.5, data.loc.w, 0, 1, true)
		vseat = data.stand
		sitting = true
		--stopTempCam()
	end
