...
AddEventHandler("Casino:SaveAmbientPeds", function(buffer)
    ...
    local fileNames = {"peddata.txt", "peddata_gabz.txt", "peddata_gabz.txt", "peddata_k4mb1.txt", "peddata_gtao.txt",
                       "0", "peddata_oldk4mb1.txt", "0", "peddata_cawles.txt"}
...
AddEventHandler("Casino:GetInfo", function()
    ...
    TriggerClientEvent("Casino:InitialInfo", playerId, GameStates, isAdmin, playerId, isOpen, nextOpenTime, FORCE_CLOSED)
end)
...
function CheckForCasinoMaps()
    local mapDependencies = {}
    mapDependencies[1] = {"rcore_casino_map", "DLCiplLoader"}
    mapDependencies[2] = {"cfx-gabz-casino"}
    mapDependencies[3] = {"nopixel4_casino"}
    mapDependencies[4] = {"k4mb1-casino"}
    mapDependencies[5] = {"rcore_casino_interior"}
    mapDependencies[7] = {"casinointerior", "casinoexterior"}
    mapDependencies[10] = {"molo_casino"}

    print("List of detected casino maps:")
    local extraMapFound = nil
    for k, v in pairs(mapDependencies) do
        for k2, v2 in pairs(v) do
            local s = GetResourceState(v2)
            if s == "started" or s == "starting" then
                if Config.MapType ~= k then
                    extraMapFound = v2
                end
                print("Found map: ^3" .. v2 .. "^7 (MapType " .. k .. ")")
            end
        end
    end
    if extraMapFound then
        print("^1[Casino] You should disable resource " .. extraMapFound ..
                  ", because casino has been set to use another map.^7")
        print("^1[Casino] You should remove '" .. extraMapFound .. "' from dependencies in fxmanifest.lua.^7")
    end
end
...
RegisterCommand("casinomaps", function(source, args, rawCommand)
    CheckForCasinoMaps()
end)
...
CreateThread(function()
    ...
    while true do
        ...
        if lastOpenState ~= isOpen then
            ...
            local nextOpenTime = GetCasinoNextOpenTime()
            TriggerClientEvent("Casino:CasinoOpenStateChanged", -1, isOpen, nextOpenTime, FORCE_CLOSED)
        end
        Wait(1000)
    end
end)
