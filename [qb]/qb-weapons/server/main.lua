...
QBCore.Functions.CreateCallback('qb-weapons:server:RepairWeapon', function(source, cb, RepairPoint, data)
    ...
    if not exports['ps-inventory']:RemoveItem(src, data.name, 1, data.slot, 'qb-weapons:server:RepairWeapon') then
        Player.Functions.AddMoney('cash', Config.WeaponRepairCosts[WeaponClass], 'qb-weapons:server:RepairWeapon')
        return
    end

    TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items[data.name], 'remove')
    TriggerClientEvent('ps-inventory:client:CheckWeapon', src, data.name)
...
QBCore.Functions.CreateCallback('prison:server:checkThrowable', function(source, cb, weapon)
    ...
    for _, v in pairs(Config.Throwables) do
        if QBCore.Shared.Weapons[weapon].name == 'weapon_' .. v then
            if not exports['ps-inventory']:RemoveItem(source, 'weapon_' .. v, 1, false, 'prison:server:checkThrowable') then return cb(false) end
            throwable = true
            break
        end
    end
...
RegisterNetEvent('qb-weapons:server:TakeBackWeapon', function(k)
    ...
    exports['ps-inventory']:AddItem(src, itemdata.name, 1, false, itemdata.info, 'qb-weapons:server:TakeBackWeapon')
    TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items[itemdata.name], 'add')
...
RegisterNetEvent('qb-weapons:server:removeWeaponAmmoItem', function(item)
    ...
    exports['ps-inventory']:RemoveItem(source, item.name, 1, item.slot, 'qb-weapons:server:removeWeaponAmmoItem')
end)
...
local function EquipWeaponTint(source, tintIndex, item, isMK2)
    ...
    Player.Functions.SetInventory(Player.PlayerData.items, true)
    exports['ps-inventory']:RemoveItem(source, item, 1, false, 'qb-weapon:EquipWeaponTint')
    TriggerClientEvent('ps-inventory:client:ItemBox', source, QBCore.Shared.Items[item], 'remove')
...
local function EquipWeaponAttachment(src, item)
    ...
    if shouldRemove then
        exports['ps-inventory']:RemoveItem(src, item, 1, false, 'qb-weapons:EquipWeaponAttachment')
    ...
    if Inventory[WeaponData.slot] then
        if Inventory[WeaponData.slot].info.attachments and next(Inventory[WeaponData.slot].info.attachments) then
            ...
            if HasAttach then
                ...
                Player.Functions.SetInventory(Player.PlayerData.items, true)
                exports['ps-inventory']:AddItem(src, AttachmentData.attachment, 1, false, false, 'qb-weapons:server:RemoveAttachment')
                TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items[AttachmentData.attachment], 'add')
