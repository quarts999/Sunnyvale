...
RegisterNetEvent('qb-ambulancejob:elevator_garage', function()
    local ped = PlayerPedId()
    for i = 1, #Config.Locations['garage'] do
        DoScreenFadeOut(500)
        while not IsScreenFadedOut() do Wait(10) end
        currentHospital = i
...
RegisterNetEvent('qb-ambulancejob:elevator_main2', function()
    local ped = PlayerPedId()
    for i = 1, #Config.Locations['main2'] do
        DoScreenFadeOut(500)
        while not IsScreenFadedOut() do Wait(10) end
        currentHospital = i
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for i = 1, #Config.Locations['main2'] do
            local v = Config.Locations['main2'][i]
            exports['qb-target']:AddBoxZone('main2' .. i, vector3(v.x, v.y, v.z), 2, 2, {
                name = 'main2' .. i,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
        ...
        for i = 1, #Config.Locations['garage'] do
            local v = Config.Locations['garage'][i]
            exports['qb-target']:AddBoxZone('garage' .. i, vector3(v.x, v.y, v.z), 2, 2, {
                name = 'garage' .. i,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
    ...
    CreateThread(function()
        ...
        for i = 1, #Config.Locations['main2'] do
            local v = Config.Locations['main2'][i]
            main2Poly[#main2Poly + 1] = BoxZone:Create(vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = 'main2' .. i,
                debugPoly = false,
                heading = 70,
                minZ = v.z - 2,
        ...
        local main2Combo = ComboZone:Create(main2Poly, { name = 'main2Poly', debugPoly = false })
        main2Combo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'ambulance' then
                if onDuty then
                    exports['qb-core']:DrawText(Lang:t('text.elevator_garage'), 'left')
                    EMSControls('garage')
                else
                    exports['qb-core']:DrawText(Lang:t('error.not_ems'), 'left')
                end
        ...
        for i = 1, #Config.Locations['garage'] do
            local v = Config.Locations['garage'][i]
            garagePoly[#garagePoly + 1] = BoxZone:Create(vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = 'garage' .. i,
                debugPoly = false,
                heading = 70,
                minZ = v.z - 2,
        ...
        local garageCombo = ComboZone:Create(garagePoly, { name = 'garagePoly', debugPoly = false })
